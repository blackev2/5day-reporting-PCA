...
font_20b <- list(family = "Arial", size = 20, color = "black")
colors_hex <- c("#332288", "#88CCEE", "#44AA99", "#117733", "#999933", "#DDCC77", "#CC6677", "#882255", "#AA4499")
colors_hex <- sort(colors_hex)
colors_hex <- c("#000000", "#322DA8", "#0077C8", "#00A378", "#B12E0F", "#ED6C21", "#000000", "#882255", "#117733")
# colors_hex <- sort(colors_hex)
...
fig <- plot_ly(data=pca_scores,x=~PC1,y=~PC2,z=~PC3, width = 1000, height = 900) %>%
  ...
  layout(title=list(text=title, size =12),
         margin = list(t = 40, r = 10, b = 10, l = 10),
         legend = list(title = list(text = "<b>  Dose (mg/kg/day) </b>"), x = 0.9, y = 0.95,itemsizing="constant"), # keep legend marker size the same as plot. otherwise, it is smaller
         # legend = list(title = list(text = "<b>  Dose <b/>"), x = 0.9, y = 0.95, font = list(family = "arial", size = 16)), # Newly added
         font = list(family = "Arial", size = 16, color = "black"), # tick 
         scene=list(
           xaxis = list(title = list(text=xlabel,font=font_20b, font_size = 10)),
           yaxis = list(title = list(text=ylabel,font=font_20b)),
           zaxis = list(title = list(text=zlabel,font=font_20b)),
           camera = list(eye = list(x=1.5, y=1.5, z = 1.7)) # newly fix bottom out of range
...
save_image(fig, file = png_file_name)
# Create word document
bold_face <- fp_text(bold = TRUE)
pca_doc <- read_docx()

# Add 3D plots
fp_title <- fpar(ftext("PCA Analysis: Three-Dimensional and Two-Dimensional Plots of Top Three PCA Components", bold_face), run_linebreak())
  ...
  for (j in (i + 1):nplot) {
    ...
    fig <- plot_ly(data=pca_scores,x=~pca_scores[[pca_x]],y=~pca_scores[[pca_y]], width = 600, height = 600, size = 10) %>%
      ...
      layout(title=list(text=paste("<b>", chemical_label, "</b>")),
             margin = list(t = 40, r = 10, b = 10, l = 10),
             legend = list(title = list(text = "<b>  Dose (mg/kg/day) </b>"), itemsizing="constant"),
             # legend = list(title = list(text = "<b>   Dose </b>"), x = 0.9, y = 0.95, font = list(family = "arial", size = 16)), # Newly added
    ...
    save_image(fig, file = file_name2)
    ...
    header_fpar <- fpar(ftext(sprintf("Two-Dimensional Plot of PCA %s Component vs. PCA %s Component", i, j), bold_face), run_linebreak())
...
# FILE OUT !!!! REVIEW BELOW THIS!!!

print(sprintf("An MS word document '%s' is created at '%s'.", file_name_doc, getwd() #output_dir
))

#Check that the word doc is there
if (!file.exists(file_name_doc)) {
  stop(paste("Word document cannot be found:", file_name_doc))
}

#Add code to convert doc to pdf
pdf_file <- sub("\\.docx$", ".pdf", file_name_doc)

#importing the Word interface
# docx_pdf <- normalizePath(
#   file.path(output_dir, file_name_doc),
#   winslash = "\\",
#   mustWork = TRUE
# )
# 
...
# Use word to create the pdf
win32com <- reticulate::import("win32com.client")
...
#saving to pdf unusual formatting wdFromatpdf
doc$SaveAs(
  file.path(getwd(), pdf_file),
  #pdf_path, 
  FileFormat = 17L)
doc$Close(FALSE)
word$Quit()

print(sprintf("A pdf document '%s' is created at '%s'.", pdf_file, getwd()
))
#message(sprintf("A PDF copy was written to : %s", pdf_path))
